...
void CNormalEffect::render() {
	...
	DrawExtendGraph(
		int(x - (this->SIZE * percent) * 0.5), int(y - (this->SIZE * percent) * 0.5), 
		int(x + (this->SIZE * percent) * 0.5), int(y + (this->SIZE * percent) * 0.5), CImage::getInstance()->getNormalEffect(), true
	);
